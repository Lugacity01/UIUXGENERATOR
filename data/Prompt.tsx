...
- Exact layout sections (header, hero, charts, cards, lists, nav, footer, sidebar)
- Realistic data examples (never generic placeholders like "amount")
- Exact chart types if charts appear (circular progress, line chart, bar chart, stacked bar, area chart, dot, sparkline)
...
- If included, it MUST be explicit and detailed:
  ...
  - Specify which icon is ACTIVE for THIS screen
    - Active state styling: text-[var(--primary)] drop-shadow-[0_0_8px_var(--primary)]
...
Subtle depth cues through shadows and layering
...
layout patterns, spacing, typography, visual density
...
STRICT:
Do NOT introduce new patterns unless a very similar pattern already exists in existing screens.
...
Realistic sample data (prices, counts, names) consistent with existing screens
Icon names for each interactive element, following the existing icon rule
Navigation details: if navigation exists, compare state between existing screens:
same placement, same icon count, and active state
